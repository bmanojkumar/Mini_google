...
/**
 * A user interface for web-crawling objects. Usage : java CrawlerDriver url
 * saveFile [list|custom] [limit]
 */
public class CrawlerDriver {
	/**
	 * A main entry point, to be used by web administrators, for creating web
	 * indexes.
	 */
	public static void main(String[] args) {
		try {
			URL u; // HOLDS AN OBJECT OF TYPE URL
			...
			if (args.length >= 2) {
				...
				// hash - Dictionary Structure based on a Hashtable or HashMap
				// from the Java collections
				// list - Dictionary Structure based on Linked List
				// myhash - Dictionary Structure based on a Hashtable
				// implemented by the students
				// bst - Dictionary Structure based on a Binary Search Tree
				// implemented by the students
				// avl - Dictionary Structure based on AVL Tree implemented by
				// the students

				if (indexMode.equals("list") || indexMode.equals("hash")
						|| indexMode.equals("myhash")
				...
				// web = new PriorityBasedSpider(u, new Indexer(indexMode));
					...
					System.out
							.println("Invalid index mode - use either \"list\" or \"hash\"");
					System.exit(1);
				}
				...
				System.out
						.println("Usage: CrawlerDriver url index-saveFile [hash | list] [crawl limit]");
				return;
			}

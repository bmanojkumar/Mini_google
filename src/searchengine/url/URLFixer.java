...
/**
 * Helper class that is used to resolve relative url's, given a parent context
 * url
 */
public class URLFixer {
	/**
	 * Function takes the non-host part of the string and returns the directory
	 * structure only. the directory will get returned with a '/' at the end and
	 * at the beginning! the file will be returned as the plain file
	 * 
	...
	/**
	 * Function takes the parent level URL and the linkCandidate and attempts to
	 * resolve any relative linkage.
	 * 
	 * @param URL
	 *            parent - the URL page on which the link is on
	 * @param String
	 *            linkCandidate - the link we are trying to fix in String form
	 * @return URL - the valid url of the fixed link
	 */
	public static URL fix(URL parent, String linkCandidate)
		...
		if (!linkCandidate.equals("")) {
			...
			if (linkCandidate.charAt(0) == '/') {
				...
				String finalString = parent.getProtocol() + "://"
						+ parent.getHost() + arr[0] + linkCandidate;
				return new URL(finalString);
			} else {
				...
				if (linkCandidate.startsWith("..") && arr[0].equals("/")) {
					finalString = parent.getProtocol() + "://"
							+ parent.getHost() + arr[0] + "/" + linkCandidate;
				} else {
					finalString = parent.getProtocol() + "://"
							+ parent.getHost() + arr[0] + linkCandidate;
